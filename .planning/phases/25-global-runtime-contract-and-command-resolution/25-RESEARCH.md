...
1. **Should managed hooks persist plain `agent-badge` or a resolved absolute executable path?**
   Resolution: persist the plain `agent-badge` command in the managed hook contract for Phase 25. Official npm, pnpm, bun, and Yarn Classic docs all define the shared-runtime model through a bin directory on `PATH`, and Phase 25 should standardize on that contract rather than storing machine-local executable paths. [CITED: https://docs.npmjs.com/downloading-and-installing-packages-globally/][CITED: https://pnpm.io/cli/bin][CITED: https://bun.sh/docs/pm/cli/add][CITED: https://classic.yarnpkg.com/lang/en/docs/cli/global/][CITED: https://git-scm.com/docs/githooks]
   Chosen contract: use plain `agent-badge` plus strong remediation and explicit cross-package-manager verification. Do not persist absolute executable paths in tracked or generated artifacts during this phase. [VERIFIED: .planning/ROADMAP.md][VERIFIED: .planning/REQUIREMENTS.md][ASSUMED]

2. **Should Phase 25 already make diagnostics and uninstall read both legacy and shared contracts?**
   Resolution: yes. Phase 25 should be dual-read and single-write. The new shared hook contract becomes the only contract written by Phase 25 code, while diagnostics and compatibility tests continue recognizing the legacy `agent-badge:refresh` shape until Phase 27 migration work removes it deliberately. [VERIFIED: codebase grep][VERIFIED: .planning/ROADMAP.md][ASSUMED]
